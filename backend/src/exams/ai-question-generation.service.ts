...
// ponytail: three workers cut wall time without opening an unbounded rate-limit storm.
const AI_CANDIDATE_CONCURRENCY = Math.min(
  4,
  Math.max(1, Number(process.env.AI_CANDIDATE_CONCURRENCY) || 3),
);
...
export class AiQuestionGenerationService {
  ...
  async generate(
    ...
    const worker = async (): Promise<void> => {
      while (true) {
        if (accepted.length >= requestedCount) return;
          ...
          try {
            ...
            if (candidate.choiceTexts !== undefined) {
                ...
                if (serverChoiceMaterialized.kind === 'accepted') {
                  ...
                  if (serverChoiceValidation.passed) {
                    admitted = {
                      ...
                      fingerprint: aiQuestionFingerprint(
                        serverChoiceMaterialized.question,
                      ),
                      attempt,
                    };
              ...
              if (
                ...
                (verification !== undefined &&
                  verification.answerIndex !== blueprint.answerIndex) ||
                (verification !== undefined &&
                  verification.choices.length !== 5) ||
                (verification !== undefined &&
                  verification.choices.filter((choice) => choice.correct)
                    .length !== 1) ||
                (verification !== undefined &&
                  !verification.choices.every(
                    (choice) =>
                      choice.correct ===
                      (choice.index === blueprint.answerIndex),
                  ))
              ) {
            ...
            if (!validation.passed) {
              ...
              if (
                ...
                const fallback = materializeSourcePreservingFallback(
                  blueprint,
                  candidate,
                );
        ...
        if (admitted !== undefined && accepted.length < requestedCount) {
    ...
    await Promise.all(
      Array.from(
        { length: Math.min(AI_CANDIDATE_CONCURRENCY, Math.max(1, total)) },
        () => worker(),
      ),
    );
    ...
    for (const rejection of rejected) {
      ...
      rejectionsByCode[rejection.code] =
        (rejectionsByCode[rejection.code] ?? 0) + 1;
    }
    return {
      requestedCount,
      accepted,
