...
export class StimulusNormalizer {
  ...
  public normalizeStimulusWithTemplate(data: any, template: string, comboBlock?: any): { stimulusData: any; effectiveTemplate: string } {
    ...
    return { stimulusData: normalized, effectiveTemplate: declaredTpl };
  }
  ...
  public detectTpl(data: any): TplName | null {
    ...
    // === 구조 기반 탐지만 유지 (내용 기반 추측 제거) ===
    if (Array.isArray(data.rows) && Array.isArray(data.headers) && data.rows.length > 0) return 'TPL_COMPARATIVE_MATRIX';
    if (data.instructor && typeof data.instructor === 'object' && data.canvas_content && typeof data.canvas_content === 'object') return 'TPL_INSTRUCTIONAL_SCENE';
    if (Array.isArray(data.participants) && Array.isArray(data.messages)) return 'TPL_CONVERSATIONAL_FLOW';
    if (data.case_profile && typeof data.case_profile === 'object' && typeof data.narrative === 'string') return 'TPL_CASE_DIAGNOSTIC_FRAME';
    if (Array.isArray(data.steps) && data.orientation) return 'TPL_SEQUENTIAL_WORKFLOW';
    if (typeof data.forum_name === 'string' && data.main_post && typeof data.main_post === 'object') return 'TPL_DIGITAL_FORUM_INTERFACE';
    if (typeof data.chart_type === 'string' && Array.isArray(data.axes) && Array.isArray(data.datasets)) return 'TPL_QUANTITATIVE_CHART';
    if (typeof data.slogan === 'string' && Array.isArray(data.bullets)) return 'TPL_PROMOTIONAL_CANVAS';
    if (typeof data.doc_type === 'string' && Array.isArray(data.paragraphs) && data.paragraphs.length > 0) return 'TPL_FORMAL_DOCUMENT';
    ...
    switch (tpl) {
      ...
      case 'TPL_FORMAL_DOCUMENT':
        ...
        if (result.paragraphs.length === 0) {
          result.paragraphs = [{ sub_title: '', content: '(내용 없음)' }];
        }
        break;
      ...
      case 'TPL_CONVERSATIONAL_FLOW':
        ...
        if (result.messages.length === 0) {
          result.participants = [{ id: 'p1', name: '', role: '' }];
      ...
      case 'TPL_CASE_DIAGNOSTIC_FRAME':
        ...
        if (!result.case_profile?.name && !result.case_profile?.context && !result.narrative) {
          result.narrative = '(내용 없음)';
        }
        break;
      ...
      case 'TPL_INSTRUCTIONAL_SCENE':
        ...
        if (!result.instructor?.text && !result.canvas_content?.data) {
          result.canvas_content = { type: 'text', data: '(내용 없음)' };
        }
        break;
      ...
      case 'TPL_DIGITAL_FORUM_INTERFACE':
        ...
        if (!result.main_post?.content) {
          result.main_post.content = result.forum_name || '(내용 없음)';
        }
        break;
