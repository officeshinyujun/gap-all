...
export function validateItems(
  ...
  for (const item of rawItems) {
    try {
      ...
      if (tplErrors.length > 0) {
        logger.warn(`TPL 스키마 오류 (${meta.recommended_template}): ${tplErrors.join('; ')}`);
      }

      const difficultyMap: Record<string, Difficulty> = {
        하: Difficulty.LOW, 중: Difficulty.MIDDLE, 상: Difficulty.HIGH,
        LOW: Difficulty.LOW, MIDDLE: Difficulty.MIDDLE, HIGH: Difficulty.HIGH,
      ...
      valid.push({
        ...
        difficulty: difficultyMap[meta.difficulty ?? meta.item_type] ?? Difficulty.MIDDLE,
  ...
  if (valid.length === 0) {
    throw new InternalServerErrorException('유효한 문항이 생성되지 않았습니다.');
  }

  return valid;
}

export function validateCombinationEncoding(item: any): { valid: boolean; reason?: string } {
  ...
  if (judgmentMap && choiceEncodingPlan?.correct_combination) {
    const correctIdx = Number(item.correct_answer ?? rr.correct_answer ?? 0) - 1;
...
export function validateStemPattern(item: any): { valid: boolean; reason?: string } {
  const rr = item.render_ready ?? {};
...
export function validateItemLogic(item: any): { valid: boolean; reason?: string } {
  const itemStructure = item.item_structure;
  const rr = item.render_ready ?? {};
  const optionsList: string[] =
    rr.options_list ?? (rr.options ?? []).map((o: any) => o.text ?? String(o));
