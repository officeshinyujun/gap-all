...
export class ChatAiService {
  ...
  async getResponse(
    ...
    // ponytail: < 200자면 교과 컨텍스트가 사실상 없는 것으로 간주하고 GPT-4o 자체 지식 허용
    const hasSufficientContext = (textbookContext?.trim().length ?? 0) >= 200;
    this.logger.debug(
      `컨텍스트 상태: ${hasSufficientContext ? '충분' : '부족'} (${textbookContext?.length ?? 0}자)`,
    );
    ...
    const systemPrompt = [
      ...
      ...(hasSufficientContext
        ...
        : [
            `## 답변 원칙 (⚠️ 교과 컨텍스트 부족 — 당신의 학습 지식을 적극 활용하세요)`,
            `1. "${subjectTitle}" 과목의 "${unitRange}" 범위에 대해 당신이 가진 지식을 최대한 활용하여 답변하세요.`,
            `2. "${unitRange}" 범위와 무관한 질문(예: 날씨, 연예인, 다른 과목)에는 "이 과목과 관련된 질문을 부탁해!"라고 자연스럽게 안내하세요.`,
            `3. 당신의 답변에 "교과 내용", "지식 베이스", "참고 자료", "청크", "검색" 같은 시스템 용어를 절대 언급하지 마세요.`,
            `4. 마치 교과서를 외우고 있는 베테랑 선생님처럼, 당당하게 가르쳐 주세요.`,
          ]),
      ``,
      ...unitRequestGuidance,
      `## 학생과의 상호작용 방식`,
      ...
      hasSufficientContext
        ...
        : `당신의 지식을 바탕으로 학생의 질문에 답변해주세요. "${subjectTitle}" 과목의 "${unitRange}" 범위를 벗어나지 마세요.`,
    ].join('\n');
