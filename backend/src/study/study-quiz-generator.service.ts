...
export class StudyQuizGeneratorService {
  ...
  async generateBlankQuestions(
    ...
    try {
      ...
      const raw = await this.textbookService.getSummationMd(
        subjectSlug,
        unitNumber,
      );
      let md: string;
      ...
      this.logger.error(
        `Blank question generation failed: ${err?.stack ?? err?.message}`,
      );
      if (err instanceof HttpException) throw err;
      throw new BadGatewayException(
        '빈칸 문제 생성에 실패했습니다. 잠시 후 다시 시도해주세요.',
      );
    }
  }
  ...
  async generateConceptPairs(
    ...
    try {
      ...
      this.logger.error(
        `Concept pair generation failed: ${err?.stack ?? err?.message}`,
      );
      if (err instanceof HttpException) throw err;
      throw new BadGatewayException(
        '개념 페어 생성에 실패했습니다. 잠시 후 다시 시도해주세요.',
      );
    }
  }
  ...
  async clearCache(
    ...
    if (process.env.DB_PROVIDER === 'local') {
      ...
      this.logger.log(
        `캐시 삭제: ${subject} ${unitNumber}단원 ${type ?? 'all'}`,
      );
      return;
    }
...
# ⚠️ 최우선 규칙 (어기면 모든 문항 폐기)
1. **correct_answer는 반드시 [교재 내용] 본문에 실제로 등장하는 단어여야 한다.** 생성하거나 추론하지 마라.
2. **sentence_template는 반드시 [교재 내용]의 문장을 거의 그대로 사용하라.** 의미를 바꾸거나 내용을 추가하지 마라. 오직 [blank] 처리만 하라.
3. **오답(options)은 반드시 같은 [교재 내용]에 등장하는 같은 분류의 용어에서 선정하라.** 외부 지식이나 교재 외 단어는 절대 사용 금지.

# 🎯 좋은 문제 vs 나쁜 문제
## ✅ 좋은 문제의 조건
- [blank]가 교재의 핵심 개념어(전문 용어, 학자명, 제도명, 분류명)이다.
- 문장 자체가 하나의 완결된 설명/정의를 담고 있어서, [blank]를 제외한 문맥만으로도 정답을 특정할 수 있다.
- 오답이 "틀렸지만 같은 맥락에서 그럴듯한" 단어들이다.
- 교재 원문을 그대로 사용하여 신뢰도가 높다.

## ❌ 나쁜 문제의 예시
- "컴퓨터는 [blank]을 처리하는 기계이다" → "정보" (일상 단어)
- "근로기준법은 [blank]을 보호하기 위한 법이다" → "근로자" (너무 뻔함)
- 오답으로 "김치", "축구", "우주" 등 교재에 없는 단어
...
→ correct_answer: "보상" (교재 본문에 실제로 등장함)
→ options: ["보상", "치료", "고용", "교육"] (모두 교재 내 같은 카테고리 용어)

# Rules
1. sentence_template는 [교재 내용]에서 발췌한 문장을 그대로 사용하거나, 최소한의 어미만 수정하라. 절대 새로운 문장을 만들지 마라.
2. [blank] 처리할 단어는 교재에 명시된 전문 용어, 학자명, 제도명, 분류명, 핵심 개념어만 가능하다. 일반 동사, 조사, 일상 명사 사용 금지.
3. sentence_template에서 [blank]가 문장의 중간에 위치해야 한다. 문장 맨 처음이나 맨 끝은 피하라.
4. options는 반드시 4개(정답 1 + 오답 3). 오답 3개는 모두 같은 [교재 내용]에 등장하는 단어에서 선정하라.
5. explanation은 교재에 적힌 내용만으로 간결하게 1-2문장으로 작성하라.

# Self-Check (출력 전 각 문항별 확인)
- [ ] correct_answer가 [교재 내용] 본문에 실제로 등장하는가? (Ctrl+F로 검증)
- [ ] sentence_template가 [교재 내용] 원문과 90% 이상 일치하는가?
- [ ] 오답 3개가 모두 [교재 내용]에 등장하는 단어인가?
- [ ] [blank] 없이도 문장만으로 정답을 특정할 수 있는가?

위 4개 항목을 모두 만족하는 문항만 출력하라.

# JSON Output
[
  {
    "id": number,
    "sentence_template": "교재 원문에서 핵심 용어만 [blank] 처리한 문장",
    "correct_answer": "[교재 내용]에 실제로 있는 정답 단어",
    "options": ["정답 포함, 모두 교재 내 동일 카테고리 용어 4개"],
...
JSON만 출력하라.`;
    ...
    const validated = this.validateBlankQuestions(arr, md);
...
JSON만 출력하라.`;
  ...
  private async writeCache<T>(
    ...
    if (process.env.DB_PROVIDER === 'local') {
      ...
      this.logger.log(
        `캐시 저장: ${subject} ${unitNumber}단원 ${type} ${count}개`,
      );
      return;
    }

    const { error } = await this.supabase.client.from('quiz_cache').upsert(
      {
        subject,
  ...
  private validateBlankQuestions(
    ...
    const validated = questions.filter((q, index) => {
      ...
      if (answer.length === 0 || answer.length > 50) {
        this.logger.warn(
          `Blank question #${q.id ?? index} rejected: invalid correct_answer length (${answer.length})`,
        );
        return false;
      }
      ...
      if (genericWords.has(answer)) {
        this.logger.warn(
          `Blank question #${q.id ?? index} rejected: correct_answer is a generic word ("${answer}")`,
        );
        return false;
      }
      ...
      if (uniqueOptions.length !== 4) {
        this.logger.warn(
          `Blank question #${q.id ?? index} rejected: options must have exactly 4 unique items (got ${uniqueOptions.length})`,
        );
        return false;
      }
      ...
      if (!uniqueOptions.includes(answer)) {
        this.logger.warn(
          `Blank question #${q.id ?? index} rejected: correct_answer not found in options`,
        );
        return false;
      }

      // 5. sentence_template must have minimum meaningful length
      const templateWithoutBlank = q.sentence_template
        .replace(/\[blank\]/g, '')
        .trim();
      if (templateWithoutBlank.length < 10) {
        this.logger.warn(
          `Blank question #${q.id ?? index} rejected: sentence too short (${templateWithoutBlank.length} chars)`,
        );
        return false;
      }

      // 6. correct_answer must appear in the source text
      const answerKey = q.correct_answer.trim().replace(/\s+/g, '');
      if (answerKey.length >= 2 && !sourceText.includes(answerKey)) {
        this.logger.warn(
          `Blank question #${q.id ?? index} rejected: correct_answer "${q.correct_answer}" not found in source text`,
        );
        return false;
      }
    ...
    if (rejected > 0) {
      this.logger.warn(
        `${rejected}/${questions.length} blank questions rejected by validation`,
      );
    }
